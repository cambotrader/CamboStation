...
def render_dashboard():
    ...
    st.title("🔮 CamboStation Regime Forecaster")

    tabs = st.tabs(["🧠 Tonal Shift Forecast", "📈 Broker Feed Sync", "📊 Emotional Drift Projection"])

    with tabs[0]:
        st.header("🧠 Forecast Next Mood Regime")
        logs = load_legacy()
    ...
    with tabs[1]:
        st.header("📈 Broker Feed (Mocked)")
        df = load_broker_data()
        st.line_chart(df["Close"])

    with tabs[2]:
        st.header("📊 Emotional Drift Projection")
...
render_dashboard()
"@ | Set-Content "$HOME\CamboStation_QuantumOS\modules\regime_forecaster.py" -Encoding UTF8

# Wire cockpit mode 31
$cockpitPath = "$HOME\CamboStation_QuantumOS\modules\cockpit.ps1"
$content = Get-Content $cockpitPath -Raw

if ($content -match '\$cmds\s*=\s*@\([^\)]*') {
    $content = $content -replace '(\$cmds\s*=\s*@\([^\)]*)', '$1,"regime forecaster"'
}
...
if ($content -notmatch '"regime forecaster"') {
    $content += "`n$patch"
}

Set-Content $cockpitPath -Value $content -Encoding UTF8
Write-Host "✅ regime_forecaster.py deployed and wired as cockpit mode 31."
