...
if (Test-Path $dashboardPath) {
    ...
    Write-Host "📁 Backup created → mission_dashboard.bak.py"
}
...
with tabs[4]:
    ...
    for belief, data in codex.items():
        st.markdown(f"- \"{belief}\" → Resonance: `{data['resonance']}` | Mutation Risk: `{data['mutation_risk']}`")
'@

Set-Content -Path $dashboardPath -Value $code -Encoding UTF8
Write-Host "`n✅ mission_dashboard.py deployed successfully."
Write-Host "▶️ Launch it with: streamlit run $dashboardPath"
